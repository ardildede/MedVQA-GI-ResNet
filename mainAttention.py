...
from models.modelAttention import VisualAttentionVQA 
...
def plot_confusion_matrix(y_true, y_pred, classes):
    ...
    plt.title('ResNet + Attention Confusion Matrix')
    plt.savefig("resnet_attn_matrix.png", dpi=300)
    print("📊 Karmaşıklık matrisi 'resnet_attn_matrix.png' olarak kaydedildi.")

def main():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"--- ResNet + Attention (VisualAttention) Başlatılıyor ---")
    print(f"Kullanılan Cihaz: {device}")
  
    raw_data = get_kvasir_data()
    train_data, val_data = get_train_val_split(raw_data, split_ratio=0.8) 
    
    print("Sözlükler hazırlanıyor...")
    ...
    print(f"-> Sınıf Sayısı: {len(answer_map)} | Kelime Sayısı: {len(vocab)}")
    ...
    MODEL_PATH = "resnet_attn_model.pth"
    ...
    report_text = classification_report(all_labels, all_preds, 
                                        ...
                                        zero_division=0)
    with open("resnet_attn_rapor.txt", "w", encoding="utf-8") as f:
        f.write(report_text)
    print("✅ 'resnet_attn_rapor.txt' kaydedildi.")
    ...
    df.to_csv("resnet_attn_analiz.csv")
    print("✅ 'resnet_attn_analiz.csv' kaydedildi (Excel formatı).")
